...
export class Character {
  ...
  #doesPositionCollideWithCollisionLayer(position) {
    ...
    // Verificar que la capa de colisión existe y tiene dimensiones
    if (!this._collisionLayer.layer || !this._collisionLayer.layer.data) {
      return false;
    }

    const tile = this._collisionLayer.getTileAtWorldXY(x, y, true);

    // Si el tile es null, significa que no hay tile en esa posición (fuera del mapa)
    if (tile === null) {
      return false; // Fuera del mapa, considerar como no colisión
    }
